package com.mostra.android.sdk.service.ble;
...
import com.mostra.android.sdk.util.MostraConstants;
...
/**
 * Mostra Android SDK 2.0.1 CQ Beacon Discovery Class. Works with Android
 * BLE library to scan and retrieve BLE devices.
 * 
...
public class MostraBLEScanService
{
	...
	private ArrayList<String> ScanRecodeParsing(byte[] scanRecord)
	{
		...
		if (scanRecord.length > 30)
		{
			...
			{
				data.add(IntToHex2(scanRecord[9] & 0xff) + IntToHex2(scanRecord[10] & 0xff) + IntToHex2(scanRecord[11] & 0xff)
						+ IntToHex2(scanRecord[12] & 0xff) + "-" + IntToHex2(scanRecord[13] & 0xff) + IntToHex2(scanRecord[14] & 0xff) + "-"
						+ IntToHex2(scanRecord[15] & 0xff) + IntToHex2(scanRecord[16] & 0xff) + "-" + IntToHex2(scanRecord[17] & 0xff)
						+ IntToHex2(scanRecord[18] & 0xff) + "-" + IntToHex2(scanRecord[19] & 0xff) + IntToHex2(scanRecord[20] & 0xff)
						+ IntToHex2(scanRecord[21] & 0xff) + IntToHex2(scanRecord[22] & 0xff) + IntToHex2(scanRecord[23] & 0xff)
						+ IntToHex2(scanRecord[24] & 0xff)); // UUID
